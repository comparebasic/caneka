...
status Config_Tests(MemCh *m){
    ...
    Node *doc = Inst_GetByPath(root, docKey->p);
    Node *tag = Inst_GetByPath(root, tagKey->p);
    Node *footer = Inst_GetByPath(root, footerKey->p);
    ...
    for(Abstract **ap = (Abstract **)arr; *ap != NULL; ap += 2){
        ...
        Abstract *value = Inst_Att(doc, key);
        args[0] = doc;
        args[1] = key;
        args[2] = expected;
        args[3] = value;
    ...
    for(Abstract **ap = (Abstract **)arr; *ap != NULL; ap += 2){
        Abstract *key = *ap;
        Abstract *expected = *(ap+1);
        Abstract *value = Inst_Att(tag, key);
        args[0] = tagKey;
        args[1] = key;
        args[2] = expected;
        args[3] = value;
    ...
    for(Abstract **ap = (Abstract **)arr; *ap != NULL; ap += 2){
        Abstract *key = *ap;
        Abstract *expected = *(ap+1);
        Abstract *value = Inst_Att(footer, key);
        args[0] = tagKey;
        args[1] = key;
        args[2] = expected;
        args[3] = value;
