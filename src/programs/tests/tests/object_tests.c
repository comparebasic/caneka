...
static status test_ObjKeyAndOrder(MemCh *m, Object *pt, i32 orderIdx, void *key, void *value){
    status r = READY;
...
status Object_Tests(MemCh *gm){
    ...
    Object_Set(pt, Str_CstrRef(m, "One"), I8_Wrapped(m, 1));
    Object_Set(pt, Str_CstrRef(m, "Two"), I8_Wrapped(m, 2));
    Object_Set(pt, Str_CstrRef(m, "Three"), I8_Wrapped(m, 3));
...
status ObjectFilter_Tests(MemCh *gm){
    ...
    Object_Set(suObj, key, I32_Wrapped(m, 3));
    Object_ByPath(obj, path, suObj, SPAN_OP_SET);

    path = StrVec_From(m, Str_FromCstr(m, "apple.beetle", ZERO));
    Path_DotAnnotate(m, path);
    suObj = Object_Make(m, TYPE_OBJECT);
    Object_Set(suObj, key, I32_Wrapped(m, 7));
    Object_ByPath(obj, path, suObj, SPAN_OP_SET);

    path = StrVec_From(m, Str_FromCstr(m, "apple.bat.cinnomon", ZERO));
    Path_DotAnnotate(m, path);
    suObj = Object_Make(m, TYPE_OBJECT);
    Object_Set(suObj, key, I32_Wrapped(m, 9));
    Object_ByPath(obj, path, suObj, SPAN_OP_SET);
    ...
    Object_Filter(obj, filterFunc, key);
