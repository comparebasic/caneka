...
status Example_ServePage(Step *st, Task *tsk){
    ...
    Route *handler = Object_ByPath(ctx->pages, ctx->path, NULL, SPAN_OP_GET);

    Object *data = Object_Make(m, ZERO);
    Object *nav = Object_Make(m, ZERO);
    Object_Set(nav, (Abstract *)Str_FromCstr(m, "pages", STRING_COPY), (Abstract *)ctx->pages);
    Object_Set(data, (Abstract *)Str_FromCstr(m, "nav", STRING_COPY), (Abstract *)nav);

    if(handler == NULL){
        ctx->code = 404;
    }else{
        Object *config = Object_GetPropByIdx(handler, ROUTE_PROPIDX_DATA);
        if(config != NULL){
            Object_Set(data,
        ...
        if((funcW->type.state & ROUTE_ASSET) == 0){
            ...
            Route *header = Object_ByPath(ctx->inc, path, NULL, SPAN_OP_GET);
