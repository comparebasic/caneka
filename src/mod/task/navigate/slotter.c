...
static gobits Slotter_CheckSlab(Slotter *q);
...
status Slotter_Remove(Slotter *q, i32 idx){
    ...
    while((Iter_Next(&it) & END) == 0){
        util *uptr = Slotter_GetCriteria(q, it.idx, idx);
        *uptr = 0;
    }
...
util *Slotter_GetCriteria(Slotter *q, i32 critIdx, i32 idx){
    ...
    SlotterCrit *crit = (SlotterCrit *)Span_Get(q->handlers, critIdx);
...
status Slotter_SetCriteria(Slotter *q, i32 critIdx, i32 idx, util **value){
    ...
    i32 slabIdx = idx / CRIT_SLAB_STRIDE;
    util *slab = NULL;
    SlotterCrit *crit = Span_Get(q->handlers, critIdx);
    if(crit == NULL){
...
static status Slotter_SetGo(Slotter *q){
    ...
    while((Iter_Next(&q->it) & END) == 0){
        if(q->handlers->nvalues > 0){
            ...
            if(q->it.idx >= u*CRIT_SLAB_STRIDE){
                ...
                Iter_Init(&it, q->handlers);
                while((Iter_Next(&it) & END) == 0){
                    SlotterCrit *crit = (SlotterCrit *)Iter_Get(&it);
...
i32 Slotter_AddHandler(Slotter *q, SlotterCrit *crit){
    Span_Add(q->handlers, crit);
    return q->handlers->max_idx;
}

Slotter *Slotter_Make(MemCh *m){
    Slotter *q = MemCh_AllocOf(m, sizeof(Slotter), TYPE_SLOTTER);
    q->type.of = TYPE_SLOTTER;
