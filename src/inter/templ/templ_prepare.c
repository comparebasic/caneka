...
status Templ_AddJump(Templ *templ,
    ...
    word one = 1;
    js->type.state |= (one << (flagIdx+8)) | (flags & (NOOP|MORE));
    return SUCCESS;
}
            ...
            if(sibIdx > -1){
                ...
                if(dest != NULL){
                    if(dest->type.state & (FETCHER_WITH|FETCHER_FOR)){
                        ...
                        r |= Templ_AddJump(templ,
                            idx, outIdx, UFLAG_ITER_FINISH_IDX, ZERO);
                    }else{
                        fch->type.state |= NOOP;
                    }
                }
