...
static status slab_ToLocal(MemCtx *m, byte dim, void *sl){
    status r = READY;
    int stride = SPAN_STRIDE;
    for(int i = 0; i < stride; i++){
        void *subSl = (void *)Slab_nextSlot(sl, i);
        if(subSl != NULL){
            slab_ToLocal(m, dim-1, subSl);
...
static status slab_FromLocal(MemCtx *m, byte dim, void *sl){
    status r = READY;
    int stride = SPAN_STRIDE;
    for(int i = 0; i < stride; i++){
        void *subSl = (void *)Slab_nextSlot(sl, i);
...
status Span_ToLocal(MemCtx *m, Abstract *a){
    ...
    r |= slab_ToLocal(m, p->dims, p->root);
...
status Span_FromLocal(MemCtx *m, Abstract *a){
    ...
    r |= slab_FromLocal(m, p->dims, p->root);
    p->m = m;
    a->type.of -= HTYPE_LOCAL;
