...
status StrVec_Tests(MemCh *gm){
    ...
    Test(Equals((Abstract *)vc, (Abstract *)s), "Testing StrVec and Str '_+' vs '_t'", vc, s);

    Str *st = Str_CstrRef(m, "time");
    Str *sa = Str_CstrRef(m, "afterwards");
    Str *sf = Str_CstrRef(m, "four");
    char *cstr = ", all alone";
    StrVec *v = StrVec_Fmt(m, "^DRy.Bold|Red|Yellow^0 then so quit '_t' '_T' _d _D _i4 _i8 _+",
    ...
    Test(Equals((Abstract *)v, (Abstract *)s), "Testing StrVec and Str");

    StrVec *vr = StrVec_ReAlign(m, v);
    Test(Equals((Abstract *)vr, (Abstract *)s), "Testing ReAligned StrVec and Str");
    Test(Equals((Abstract *)vr, (Abstract *)v), "Testing StrVec and StrVec");
