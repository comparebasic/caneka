...
status Salty_Tests(MemCtx *gm){
    ...
    DebugStack_SetRef("Testing Enc/Dec", TYPE_CSTR);
    String *keyOne = String_Make(m, bytes(cstrOne));
    ...
    DebugStack_SetRef("Testing Key Add", TYPE_CSTR);
    ...
    r |= Test(key != NULL, "'test' user can access 'test' key, have, '%s'", key != NULL ? key->bytes : NULL);

    s = Cont(m, bytes("One fell over the nest of another, things are good, and that's a never ending scenario, of life!"));

    DebugStack_SetRef("EncPair Fill", TYPE_CSTR);
    ...
    DebugStack_SetRef("Testing EncPair Enc/Dec", TYPE_CSTR);
    ...
    DebugStack_SetRef("Testing second EncPair Enc/Dec", TYPE_CSTR);
    ...
    DebugStack_SetRef("Testing EncPair Oset", TYPE_CSTR);
    ...
    char *exp ="enc/3={key:s/4=test;enc:s/24=XW7uW9kd63elQ8CChdQGCQ==;length:i64/2=11;}";
